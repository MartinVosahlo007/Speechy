...
class OmniVoiceRuntime:
    ...
    def default_voice_name(self) -> str:
        return self.voice_store.default_voice_name

    def list_voices(self) -> list[dict[str, Any]]:
        return [
            {
                **self.voice_store.serialize(path),
                "provider": self.provider_id,
                "label": path.stem,
                "kind": "wav",
            }
            for path in self.voice_store.list_voice_paths()
        ]
    ...
    def import_voice_asset(self, filename: str, content: bytes) -> dict[str, Any]:
        if not filename.lower().endswith(".wav"):
            raise ValueError("Only WAV voice files are supported.")
        saved = self.voice_store.save_upload(filename, content)
        return {
            **self.voice_store.serialize(saved),
            "provider": self.provider_id,
            "label": saved.stem,
            "kind": "wav",
        }
    ...
    def prepare_voice(self, voice_name: str, options: InferenceOptions | None = None):
        preprocess_prompt = getattr(options, "preprocess_prompt", True) if options else True
        return self.create_voice_clone_prompt(voice_name, preprocess_prompt)
    ...
    def render_single_block(
        ...
        model = self._ensure_model()
        prompt = prepared_voice or self.create_voice_clone_prompt(
            voice_name,
    ...
    def _ensure_model(self):
        ...
        if self._compile_enabled and hasattr(torch, "compile"):
            try:
                ...
                print(f"[OmniVoiceRuntime] torch.compile failed, using eager mode: {exc}", file=sys.stderr)
        return self._model
