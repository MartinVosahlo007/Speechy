...
- **2026-06-08**: Documentation accuracy pass. Architecture scaffold matches repo; hotspot limits reference `scripts/check-architecture.mjs`; REFACTOR_PLAN marks completed phases and partial backend/HTTP splits; README documents AI agent `.env` setup; docs map consolidated. Same day: repo cleanup (archive, IDE folders, caches). No constitutional rule changes.
- **2026-06-04**: Consolidated documentation layout. Added `docs/README.md` and `docs/ai-workflow.md`; moved `.planning/codebase/` and `docs/superpowers/` into `docs/archive/` (later removed 2026-06-08); slimmed `AGENTS.md`; added `.cursor/rules/speechy.mdc` and `.codex/skills/README.md`; tracked project skills under `.codex/skills/` in git. No constitutional rule changes.
